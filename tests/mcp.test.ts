...
describe("MCP protocol compliance (any CLI can connect)", () => {
  ...
  it("initializes, lists tools with valid schemas, and round-trips write+read over stdio", async () => {
    const transport = new StdioClientTransport({
      ...
      env: {
        ...process.env,
        OCTOBOARD_DIR: join(dir.path, ".octoboard"),
        OCTOBOARD_AGENT: "cursor",
      },
    });
    ...
    await client.callTool({
      name: "board_note",
      arguments: { text: "hello from a generic MCP client" },
    });
    ...
    expect(parsed.agents.map((a: { name: string }) => a.name)).toContain(
      "cursor",
    );
...
describe("mcp-config generator", () => {
  it("produces a valid mcpServers block for JSON clients", () => {
    ...
    expect(parsed.mcpServers.blackboard.args).toContain(
      "octopus-blackboard-mcp",
    );
  ...
  it("defaults the agent identity to the client name, and emits TOML for codex", () => {
    expect(
      JSON.parse(mcpConfig("gemini").content).mcpServers.blackboard.env
        .OCTOBOARD_AGENT,
    ).toBe("gemini");
