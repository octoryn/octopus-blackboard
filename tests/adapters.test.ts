...
describe("generic adapter", () => {
  it("parses the {events:[...]} object form", () => {
    const events = genericAdapter.parse(
      JSON.stringify({ events: [{ type: "note", text: "hi" }] }),
    );
  ...
  it("parses a bare array and JSONL, and drops invalid events", () => {
    expect(
      genericAdapter.parse('[{"type":"file","path":"a.ts","change":"added"}]')
        .length,
    ).toBe(1);
    const jsonl =
...
describe("claude-code heuristic adapter", () => {
  ...
  it("extracts and dedups file edits, deriving change kind from the tool", () => {
    ...
    const byPath = Object.fromEntries(
      events.map((e) => [(e as any).path, (e as any).change]),
    );
  ...
  it("handles the codex apply_patch shape via the path+edit-tool rule", () => {
    const events = getAdapter("codex")!.parse(
      '{"tool":"apply_patch","path":"lib/x.py"}',
    );
    expect(events).toEqual([
      { type: "file", path: "lib/x.py", change: "modified" },
    ]);
  });
});
